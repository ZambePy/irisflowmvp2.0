...
"""
Calibração de 9 pontos — converte gaze norm (pitch/yaw normalizado) para pixels na tela.

Fluxo:
    1. Usuário olha para 9 pontos conhecidos na tela
    2. Para cada ponto: coletar N predições do modelo (pitch_norm, yaw_norm)
    3. fit() → aprende a transformação afim norm → pixels
...
class GazeCalibrator:
    """
    Calibração leve por regressão afim (norm → pixels).

    Não modifica o modelo — aprende apenas a transformação
    do espaço normalizado para o espaço da tela do usuário.
    ...
    Uso:
        ...
        cal.add_point(
            gaze_norm=(pitch_norm, yaw_norm),   # predição média do modelo ∈ [-1, 1]
    ...
    def add_point(
        ...
        Args:
            gaze_norm: (pitch_norm, yaw_norm) ∈ [-1, 1] — predição do modelo
            target_px: (x, y) — posição do ponto na tela em pixels
        """
        if len(self._gaze_norm) >= 9:
            raise ValueError("Máximo de 9 pontos de calibração.")
        self._gaze_norm.append(gaze_norm)
        self._target_px.append(target_px)

    def point_count(self) -> int:
        return len(self._gaze_norm)
    ...
    def fit(self) -> float:
        ...
        """
        n = len(self._gaze_norm)
        if n < 4:
            raise ValueError(f"Mínimo de 4 pontos — {n} fornecidos.")

        # Fonte: pontos em espaço normalizado (com coluna de bias)
        src = np.array([[x, y, 1.0] for x, y in self._gaze_norm], dtype=np.float64)
    ...
    def convert(self, gaze_pitch_norm: float, gaze_yaw_norm: float) -> tuple[float, float]:
        """
        Converte predição do modelo (pitch_norm, yaw_norm) → pixels na tela.
        ...
        src  = np.array([gaze_pitch_norm, gaze_yaw_norm, 1.0], dtype=np.float64)
        pred = src @ self._A  # (2,)
